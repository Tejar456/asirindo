      ...
      <div v-else>
        ...
        <div
          ...
          <h4 class="text-white text-base sm:text-lg font-semibold z-10 text-center">
            ...
            >
            <nuxt-link to="/news" class="text-yellow-400">Berita</nuxt-link>
          </h4>
        </div>
      </div>
    </div>
  ...
  <div class="max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-auto">
    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
      <div v-for="(item, i) in news" :key="i"
        class="group flex flex-col focus:outline-none border-2 border-slate-200 rounded-2xl p-4 hover:border-yellow-400 transition-all duration-300 hover:shadow-lg"
        :href="`/news/${news.id}`">
